...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	sprintf(szDebugBuffer, "Valores antes de la comprobacion:\n\nAuthServer PackageType (desde DB): %d\nGameServer CLIENTE_UPDATE (desde stdafx.h): %d", this->m_AuthInfo.PackageType, GAMESERVER_CLIENTE_UPDATE);
	MessageBoxA(NULL, szDebugBuffer, "DEBUG DE LICENCIA - Ricardo", MB_OK | MB_ICONINFORMATION);
...
inline void CProtect::ErrorMessageBox(int Erro) // OK
{
	...
	wsprintf(buff,"You Need A License/Necesitas una licencia, WhatsApp: +51917039407",Erro);
