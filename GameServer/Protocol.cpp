...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
		...
		case 0xFC:
			...
			{
			case 0x06:
				gMuaVip.BUY_MUA_VIP(lpObj, (BUY_MUA_VIP_REQ*)lpMsg);
				break;
			case 0x03:
		...
		case 0xF2:
				switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
				{
					case 0x05:
						...
						break;
					case 0x06:
						gChangeReset.RecvChangeReset((CG_CHANGE_RESET_RECV*)lpMsg, aIndex);
						break;
					case 0x0B:
						gWarehousePanel.CGWarehouseOpen((PMSG_WAREHOUSEINTERFACE_RECV*)lpMsg, aIndex);
						break;
					case 0x0C:
...
{
	// ----
	PMSG_RANKTITLE_UPD pMsg;
	pMsg.h.set(0xFB, 0x35, sizeof(pMsg));
	pMsg.RankTitle1 = lpObj->rDanhHieu; 

	DataSend(lpObj->Index, (BYTE*)&pMsg, sizeof(pMsg));
}

void GetInfoRankTitle3(LPOBJ lpObj)
{
	// ----
	PMSG_MUA_VIP_UPD pMsg;

	pMsg.h.set(0xFB, 0x38, sizeof(pMsg));
	pMsg.RankTitle4 = lpObj->rMuaVip;
...
void ExtraCpanelSendInfo(int aIndex)
{
	...
	pInfo.LuckyWheelEnabled = gServerInfo.m_LuckyWheelEnabled;
	pInfo.LuckyWheelWCoinC = gServerInfo.m_LuckyWheelWCoinC;
	pInfo.LuckyWheelWCoinP = gServerInfo.m_LuckyWheelWCoinP;
	pInfo.LuckyWheelGPCoin = gServerInfo.m_LuckyWheelGPCoin;
	pInfo.LuckyWheelTime = gServerInfo.m_LuckyWheelTime;
