...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	pMsg.MarriedTest = lpObj->MarryCharacterInfo.Married;	
	#endif
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	switch(lpMsg->type)
	{
		...
		case 4:
			...
			break;
		case 5:
			PostMessage1(lpMsg->name,gMessage.GetMessage(880),lpMsg->message); // POST VIP
			break;
		case 6:
...
void GDCharacterInfoSend(int aIndex,char* name) // OK
{
	...
	pMsg.MarriedTest = gObj[aIndex].MarryCharacterInfo.Married;

	memcpy(pMsg.MarriedNameTest, name, sizeof(pMsg.MarriedNameTest));
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	pMsg.MarriedTest = lpObj->MarryCharacterInfo.Married;
	memcpy(pMsg.MarriedNameTest, lpObj->MarryCharacterInfo.MarriedName, sizeof(pMsg.MarriedNameTest));

	pMsg.rMuaVip = lpObj->rMuaVip;

	pMsg.rDanhHieu = lpObj->rDanhHieu;

	pMsg.Lvtuchan = lpObj->Lvtuchan;

	pMsg.rcrit = lpObj->rcrit;

	pMsg.rexc = lpObj->rexc;

	pMsg.rdmg = lpObj->rdmg;

	pMsg.rdouble = lpObj->rdouble;
...
{
	SET_MARRY_DIVORSE_INFOSAVE pMsg = {0};
	
	PHeadSetB((LPBYTE)&pMsg, GSSend_PlayerMarry, sizeof(pMsg));
	
	memcpy(pMsg.Name, Name, MAX_ACCOUNT_LEN);
	
	gDataServerConnection.DataSendD((char*)&pMsg, pMsg.h.size);
...
void gDanhHieuDataSend(LPOBJ lpObj)
{
	...
	// ----
	pMsg.Title2 = lpObj->Lvtuchan;
	pMsg.Title3 = lpObj->rcrit;
	pMsg.Title4 = lpObj->rexc;
	pMsg.Title5 = lpObj->rdmg;
	pMsg.Title6 = lpObj->rdouble;
