	...
	while ( true )
	{
		...
		if ( iType == 1 )
		{
			while(true)
			{
				...
				{
					ErrorMessageBox("BotStore error: BotStoreIndex: %d out of range!", BotNum);
					return;
				}
				...
				Token = GetToken();
				this->bot[BotNum].ColorName = TokenNumber;
		...
		if ( iType == 2 )
		{
			while(true)
			{
				...
				if(BotNum < 0 || BotNum > MAX_BOTSTORE-1)
				{
					LogAdd(LOG_BOT,"BotStore error: BotStoreIndex: %d doesnt exist", BotNum);
					return;
				}
		...
		if ( iType == 3 )
		{
			...
			while(true)
			{
				...
				Token = GetToken();
				this->bot[BotNum].storeItem[Num].PCPoints = TokenNumber;
...
void ObjBotStore::gObjTempInventoryItemBoxSet(BYTE * TempMap, int itempos, int xl, int yl, BYTE set_byte)
{
	...
	for(h = 0; h < 4; h++)
	{
		for(w = 0; w < 8; w++)
		{
			if(*(BYTE*)(TempMap + h * 8 + w) == 255)
			{
				...
				if(blank != 255)
				{
					this->gObjTempInventoryItemBoxSet(TempMap,blank,iwidth,iheight,type);
					return blank;
				}
			}
		}
...
void ObjBotStore::AddItem(int bIndex,int botNum)
{
	...
	{ 
		...
		if(blank != 255)
		{
			blank += MAIN_INVENTORY_NORMAL_SIZE - INVENTORY_WEAR_SIZE;
			...
			item.Coin1		   = this->bot[botNum].storeItem[i].PCPoints; // //m_PcPointValue & PCPoints
		...
		for(int botNum=0;botNum<MAX_BOTSTORE;botNum++)
		{
			if(this->bot[botNum].Enabled == true)
			{
				...
				if(result >= 0)
				{
					...
					gObj[result].IsBot = Type_Store;
					gObj[result].BotPower = 1; // 0  Maneja las monedas
					...
					gObj[result].PathCount = 0;
					gObj[result].BotPower = 1;
					gObj[result].BotDefense = 1;
					...
					gObj[result].PKLevel = this->bot[botNum].ColorName;
					gObj[result].MoveRange = 1; // 1
...
BYTE ObjBotStore::gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck) //OK
{
