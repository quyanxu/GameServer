...
void CCustomRankUser::GCReqRankLevelUser(int aIndex, int aTargetIndex)
{
	...
	pMsg.m_RankTitle4 = gObj[aTargetIndex].rMuaVip;
	pMsg.iLvtuchan		= gObj[aTargetIndex].Lvtuchan;
	pMsg.ircit = gObj[aTargetIndex].rcrit;
	pMsg.irexc = gObj[aTargetIndex].rexc;
	pMsg.irdmg = gObj[aTargetIndex].rdmg;
	pMsg.irdouble = gObj[aTargetIndex].rdouble;
