...
if( Conectar == 1 )
	if(gServerInfo.m_ServerMaxUserNumber <= 100) // CAMBIADO A 10 /100
	{
		wsprintf(text, "ONLINE: %d/%d", gObjTotalUser,gServerInfo.m_ServerMaxUserNumber);
	}
	else
	{
		wsprintf(text, "ONLINE: %d/%d", gObjTotalUser,10);
	}
else
{
	wsprintf(text, "ONLINE: %d/%d", gObjTotalUser,gServerInfo.m_ServerMaxUserNumber);
...
void CServerDisplayer::PaintInvasionTime() // OK
{
	...
	for (int n=0; n < 26; n++)
	{
		...
		if (n==14){ wsprintf(text1,"%d- %s",n, "Protector BC: "); }
		if (n==15){ wsprintf(text1,"%d- %s",n, "Custom 8: "); }
		if (n==16){ wsprintf(text1,"%d- %s",n, "Custom 9: "); }
		if (n==17){ wsprintf(text1,"%d- %s",n, "Custom 10: "); }
		if (n==18){ wsprintf(text1,"%d- %s",n, "Custom 11: "); }
		if (n==19){ wsprintf(text1,"%d- %s",n, "Custom 12: "); }
		if (n==20){ wsprintf(text1,"%d- %s",n, "Custom 13: "); }
		if (n==21){ wsprintf(text1,"%d- %s",n, "Custom 14: "); }
		if (n==22){ wsprintf(text1,"%d- %s",n, "Custom 15: "); }
		if (n==23){ wsprintf(text1,"%d- %s",n, "Custom 16: "); }
		if (n==24){ wsprintf(text1,"%d- %s",n, "Custom 17: "); }
		if (n==25){ wsprintf(text1,"%d- %s",n, "Custom 18: "); }
